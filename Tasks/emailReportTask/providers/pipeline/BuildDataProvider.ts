...
export class BuildDataProvider implements IDataProvider {
  ...
  public async getReportDataAsync(pipelineConfig: PipelineConfiguration, reportDataConfiguration: ReportDataConfiguration): Promise<Report> {
    ...
    const timeline = await RetryHelper.RetryAsync(() => this.pipelineRestClient.getPipelineTimelineAsync(build.id));
    const changes = await RetryHelper.RetryAsync(() => this.pipelineRestClient.getPipelineChangesAsync(build.id));
    ...
    const lastCompletedTimeline = lastCompletedBuild != null ? await RetryHelper.RetryAsync(() => this.pipelineRestClient.getPipelineTimelineAsync(lastCompletedBuild.id)) : null;
  ...
  private async getBuildAsync(pipelineConfig: PipelineConfiguration): Promise<Build> {
    var build = await RetryHelper.RetryAsync(this.pipelineRestClient.getPipelineAsync);
