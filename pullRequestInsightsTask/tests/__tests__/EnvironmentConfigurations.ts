...
describe("EnvirnmentConfigurations Tests", () => {
  ...
  function setUpGetVariableCall(
    parametersToOutputs: Map<string, string>
  ): void {
  ...
  test("Gets pull request id from system", () => {
    setUpGetVariableCall(
      new Map<string, string>([
        ["SYSTEM_PULLREQUEST_PULLREQUESTID", "9"],
        ["BUILD_SOURCEBRANCH", "ref/pull/11/master"]
      ])
    );
  ...
  test("Gets pull request id from source branch", () => {
    setUpGetVariableCall(
      new Map<string, string>([
        ["SYSTEM_PULLREQUEST_PULLREQUESTID", undefined],
        ["BUILD_PULLREQUEST_ID", undefined],
        ["BUILD_SOURCEBRANCH", "ref/pull/11/master"]
      ])
    );
  ...
  test("Gets no pull request id when pipeline is not result of pull request", () => {
    setUpGetVariableCall(
      new Map<string, string>([
        ["SYSTEM_PULLREQUEST_PULLREQUESTID", undefined],
        ["BUILD_PULLREQUEST_ID", undefined],
        ["BUILD_SOURCEBRANCH", "ref/11/master"]
      ])
    );
