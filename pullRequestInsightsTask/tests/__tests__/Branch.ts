...
describe("Branch Tests", () => {
  ...
  function makeTask(
    ...
    sinon.stub(fake, "getType").returns("type");
    return fake;
  }
  ...
  test("Null return when invalid task id is given", () => {
    branch = new Branch("", [
      makePipeline(undefined, undefined, []),
      makePipeline(undefined, undefined, []),
      makePipeline(undefined, undefined, [])
    ]);
    expect(
      branch.getPercentileTimeForPipelineTask(70, "abc", "id", "type")
    ).toBeNull();
  });

  test("Correct percentile is returned when task only ran on some pipelines", () => {
    branch = new Branch("", [
      makePipeline(undefined, undefined, [makeTask("abc", "id", [16])]),
      makePipeline(undefined, undefined, [makeTask("abc", "id", null)]),
      makePipeline(undefined, undefined, [makeTask("abc", "id", [4])]),
      makePipeline(undefined, undefined, [makeTask("abc", "id", [20])]),
      makePipeline(undefined, undefined, [makeTask("abc", "id", [3])])
    ]);
    expect(
      branch.getPercentileTimeForPipelineTask(75, "abc", "id", "type")
    ).toBeCloseTo(18);
  });

  test("Correct percentile is returned for a valid task when percentile falls on exact length", () => {
    branch = new Branch("", [
      makePipeline(undefined, undefined, [makeTask("jkl", "id", [4])]),
      makePipeline(undefined, undefined, [makeTask("jkl", "id", [2])]),
      makePipeline(undefined, undefined, [makeTask("jkl", "id", [3])]),
      makePipeline(undefined, undefined, [makeTask("jkl", "id", [1])])
    ]);
    expect(
      branch.getPercentileTimeForPipelineTask(62.5, "jkl", "id", "type")
    ).toBeCloseTo(3);
  });
  ...
  test("Correct percentile is returned for a valid task when percentile does not fall on exact length", () => {
    ...
    expect(
      branch.getPercentileTimeForPipelineTask(40, "jkl", "id", "type")
    ).toBeCloseTo(2.1);
  });
