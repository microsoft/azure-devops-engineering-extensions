...
describe('BuildTask Tests', () => {
    ...
    test("Task failed when it has failure status", () => {
        task = new BuildTask(null, "", new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, azureBuildInterfaces.TaskResult.Failed);
        expect(task.wasFailure()).toBe(true);
    });
    test("Task did not fail when it does not have failure status", () => {
        task = new BuildTask(null, "", new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, azureBuildInterfaces.TaskResult.Succeeded);
        expect(task.wasFailure()).toBe(false);
    });

    test("Correct duration is calculated", () => {
        task = new BuildTask(null, null, new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, null, null);
        sinon.stub(task, "ran").returns(true);
    ...
    test("Task is long running when ran took longer than threshold time", () => {
        task = new BuildTask(null, "abc", null, null, null, null, null);
    ...
    test("Task ran when it is complete and has times", () => {
        task = new BuildTask(null, null, new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, null);
        expect(task.ran()).toBe(true);
    });

    test("Task did not run when incomplete", () => {
        task = new BuildTask(null, null, new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.InProgress, null);
        expect(task.ran()).toBe(false);
    });

     test("Task did not run when it has no start time", () => {
        task = new BuildTask(null, null, null, new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, null);
        expect(task.ran()).toBe(false);
    });

    test("Task did not run when it has no end time", () => {
        task = new BuildTask(null, null, new Date("2019-05-23 01:14:40.00"), null, null, azureBuildInterfaces.TimelineRecordState.Completed, null);
        expect(task.ran()).toBe(false);
    });
    ...
    test("Tasks are equal when they have same id and name", () => {
       task = new BuildTask({id: "abc"},"123", null, null, null, azureBuildInterfaces.TimelineRecordState.InProgress, null);
       expect(task.isInstanceOfTask("123", "abc")).toBe(true);
    });

    test("Tasks are not equal when ids are different", () => {
        task = new BuildTask({id: "13"}, "abc", null, null, null, null, null);
        expect(task.isInstanceOfTask("abc", "123")).toBe(false);
    });

    test("Tasks are not equal when names are different", () => {
        task = new BuildTask({id: "123"}, "ab", null, null, null, null, null);
        expect(task.isInstanceOfTask("abc", "123")).toBe(false);
    });
