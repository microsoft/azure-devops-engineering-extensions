...
describe("PipelineTask Tests", () => {
    ...
    test("Shortest duration is correct when there is a single instance",  () => {
       task.addTaskInstance(makeFakeTaskRun("abc", "123", "type", 5, null, null, true));
       expect(task.getShortestRegressiveDuration()).toBe(5);
    });

    test("Longest regression is correct when there is a single instance",  () => {
        task.addTaskInstance(makeFakeTaskRun("abc", "123", "type", 5, 2, null, true));
    ...
    test("Longest duration is correct when there are multiple instances",  () => {
        addTasksToValidation([makeFakeTaskRun("abc", "123", "type", 5, null, null, true), makeFakeTaskRun("abc", "123", "type", 7, null, null, true), makeFakeTaskRun("abc", "123", "type", 6, null, null, true)]);
    ...
    test("Longest regression is correct when there are multiple instances",  () => {
        addTasksToValidation([makeFakeTaskRun("abc", "123", "type", 5, 3, null, true), makeFakeTaskRun("abc", "123", "type", 7, 5, null, true), makeFakeTaskRun("abc", "123", "type", 6, 4, null, true)]);
    ...
    test("Regressive extremes only consider long running instances",  () => {
        addTasksToValidation([makeFakeTaskRun("abc", "123", "type", 5, 3, null, false), makeFakeTaskRun("abc", "123", "type", 7, 5, null, true), makeFakeTaskRun("abc", "123","type",  6, 4, null, false)]);
    ...
    test("Number of agents task ran on is properly calculated",  () => {
        addTasksToValidation([makeFakeTaskRun("abc", "123", "type", null, null, "a"), makeFakeTaskRun("abc", "123", "type", null, null, "b"), makeFakeTaskRun("abc", "123", "type", null, null, "c")]);
