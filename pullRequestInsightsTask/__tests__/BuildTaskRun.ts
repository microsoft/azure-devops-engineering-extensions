...
describe('BuildTaskRun Tests', () => {
    ...
    test("Task failed when it has failure status", () => {
        task = new BuildTaskRun(null, "", new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, azureBuildInterfaces.TaskResult.Failed);
        expect(task.wasFailure()).toBe(true);
    });
    test("Task did not fail when it does not have failure status", () => {
        task = new BuildTaskRun(null, "", new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, azureBuildInterfaces.TaskResult.Succeeded);
        expect(task.wasFailure()).toBe(false);
    });

    test("Correct duration is calculated", () => {
        task = new BuildTaskRun(null, null, new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, null, null);
        sinon.stub(task, "ran").returns(true);
    ...
    test("Task ran when it is complete and has times", () => {
        task = new BuildTaskRun(null, null, new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, null);
        expect(task.ran()).toBe(true);
    });

    test("Task did not run when incomplete", () => {
        task = new BuildTaskRun(null, null, new Date("2019-05-23 01:14:40.00"), new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.InProgress, null);
        expect(task.ran()).toBe(false);
    });

     test("Task did not run when it has no start time", () => {
        task = new BuildTaskRun(null, null, null, new Date("2019-05-24 02:15:55.00"), null, azureBuildInterfaces.TimelineRecordState.Completed, null);
        expect(task.ran()).toBe(false);
    });

    test("Task did not run when it has no end time", () => {
        task = new BuildTaskRun(null, null, new Date("2019-05-23 01:14:40.00"), null, null, azureBuildInterfaces.TimelineRecordState.Completed, null);
        expect(task.ran()).toBe(false);
    });
